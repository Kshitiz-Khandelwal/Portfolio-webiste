...
export function WhatIWorkOn() {
  ...
  return (
    <section id="what-i-work-on" className="py-20 px-4 sm:px-6 max-w-7xl mx-auto border-t border-[#E0C9AE]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {domains.map((d) => {
          ...
          return (
            <div
              key={d.id}
              className="relative bg-[#FDF8F2] border border-[#E0C9AE] hover:border-[#E8846B] rounded-3xl p-6 transition-all duration-300 hover:-translate-y-1.5 hover:shadow-warmLg shadow-warm flex flex-col justify-between group"
            >
              <div>
                <div className="flex items-center justify-between pb-2 mb-3 border-b border-[#E0C9AE]">
                  <span className="font-mono text-[10px] text-[#8A7A63] font-bold">
                    {d.page}
                  </span>
                  <span className={`font-mono text-[9px] uppercase tracking-wider px-2 py-0.5 rounded-full font-bold ${d.badgeClass}`}>
                    {d.badge}
                  </span>
                </div>
                ...
                <div className="flex items-center gap-3 mb-3">
                  <div
                    ...
                    style={{
                      ...
                      color: d.color === "#8FBFA0" ? "#2D5E40" : "#B5432C",
                    }}
                  >
                    <Icon className="w-5 h-5" />
                  </div>
                  <h3 className="font-sans font-bold text-base sm:text-lg text-[#3A2E22] group-hover:text-[#E8846B] transition-colors leading-tight">
                    {d.title}
                  </h3>
                </div>

                <p className="font-sans text-xs text-[#5C4D3C] leading-relaxed mb-6">
                  {d.description}
                </p>
              </div>

              <div>
                <div className="flex flex-wrap gap-1.5 pt-4 border-t border-[#E0C9AE]">
                  {d.topics.map((t, tIdx) => (
                    <span
                      key={t}
