...
export function MentorshipTestimonials() {
  ...
  return (
    <section className="py-16 px-4 sm:px-6 max-w-7xl mx-auto bg-[#271A38] rounded-3xl my-8 border border-[#3D2B55]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {testimonials.map((t, idx) => (
          <div
            key={idx}
            className="relative bg-[#1F142D] border border-[#3D2B55] hover:border-[#A78BFA] rounded-3xl p-6 sm:p-7 shadow-violet hover:shadow-violet-lg transition-all duration-300 flex flex-col justify-between group"
          >
            <div>
              <div className="flex items-center justify-between pb-3 mb-4 border-b border-[#3D2B55]">
                ...
                <Quote className="w-5 h-5 text-[#A78BFA] opacity-80" />
              </div>

              <p className="font-sans text-xs sm:text-sm text-[#C4B5FD] leading-relaxed italic mb-6">
                &ldquo;{t.quote}&rdquo;
              </p>
            </div>

            <div className="pt-4 border-t border-[#3D2B55]">
              <h4 className="font-sans font-bold text-sm text-[#E5DAF7] group-hover:text-[#A78BFA] transition-colors">
                {t.author}
              </h4>
              <p className="font-sans text-xs text-[#A78BFA] font-medium">
                {t.role}
              </p>
              <p className="font-sans text-[11px] text-[#A99BC2] mt-0.5">
                {t.institution}
              </p>
            </div>
          </div>
