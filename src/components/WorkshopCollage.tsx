...
export function WorkshopCollage() {
  ...
  return (
    <div className="mt-10 bg-[#1F142D] border border-[#3D2B55] rounded-3xl p-6 sm:p-10 shadow-violet relative overflow-hidden">
      {/* Header Bar */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 mb-8 border-b border-[#3D2B55] relative z-10">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <h3 className="font-sans font-bold text-xl sm:text-2xl text-[#E5DAF7]">
              Workshop Idea Gallery &amp; Photo Documentation
            </h3>
          </div>
          <p className="font-sans text-xs sm:text-sm text-[#A99BC2]">
            ...
            <button
              ...
              className={`px-3.5 py-1.5 rounded-xl text-xs font-mono font-medium transition-all ${
                activeFilter === tab
                  ...
                  : "bg-[#271A38] text-[#C4B5FD] hover:text-white border border-[#3D2B55]"
              }`}
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 relative z-10">
        {filtered.map((item) => (
          <div
            key={item.src}
            className="group relative bg-[#271A38] border border-[#3D2B55] hover:border-[#A78BFA] p-3.5 rounded-2xl shadow-violet hover:shadow-violet-lg transition-all duration-300 hover:-translate-y-1 cursor-pointer"
            onClick={() => setZoomedPhoto(item)}
          >
            {/* Photo Frame */}
            <div className="relative aspect-[4/3] rounded-xl overflow-hidden bg-[#1A1025] border border-[#3D2B55] mb-3">
              <Image
                src={item.src}
                alt={item.title}
                fill
              ...
              <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                <span className="px-3 py-1.5 rounded-lg bg-[#1A1025] text-xs font-sans font-medium text-[#E5DAF7] border border-[#3D2B55] flex items-center gap-1.5 shadow-violet">
                  <ZoomIn className="w-3.5 h-3.5 text-[#A78BFA]" />
            ...
            <div className="pt-1 px-1">
              <div className="flex items-center justify-between gap-1 mb-1">
                <span className="font-mono text-[9px] uppercase tracking-wider px-2 py-0.5 rounded-md badge-violet font-semibold">
                  {item.venue} Campus
                </span>
                <span className="font-mono text-[9px] text-[#34D399]">✓ Verified Session</span>
              </div>
              <h4 className="font-sans font-bold text-sm text-[#E5DAF7] group-hover:text-[#A78BFA] transition-colors leading-snug">
                {item.title}
              </h4>
              <p className="font-sans text-xs text-[#A99BC2] mt-1 line-clamp-2">
                {item.caption}
              </p>
            </div>
          </div>
      ...
      {zoomedPhoto && (
        ...
        >
          ...
          >
            <div className="relative w-full aspect-[4/3] rounded-2xl overflow-hidden bg-[#1A1025] border border-[#3D2B55] mb-4">
              <Image
                src={zoomedPhoto.src}
            ...
            <div className="flex items-center justify-between pt-2 border-t border-[#3D2B55]">
              <div>
                <span className="font-mono text-[10px] uppercase font-semibold text-[#A78BFA]">
                  {zoomedPhoto.venue} Campus Session
                </span>
                <h3 className="font-sans font-bold text-lg text-[#E5DAF7]">
                  {zoomedPhoto.title}
                </h3>
                <p className="font-sans text-xs text-[#A99BC2] mt-0.5">
                  {zoomedPhoto.caption}
                </p>
              </div>
            </div>
